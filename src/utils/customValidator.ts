...
export function checkValueType(
...
): { status: 'INVALID' | 'EMPTY'; msg: string } | null {
  const intRegex = /^\d+$/;
  ...
  if (type !== 'bool' && !value) {
    return {
      status: 'INVALID',
      msg: $localize`:@@101:Value should not be empty`,
    };
  }
  ...
  if (type.toLowerCase() == 'float') {
    if (!floatRegex.test(value)) {
      return {
        status: 'INVALID',
        msg: $localize`:@@102:Value is not a valid ${type}`,
      };
    } else if (constraints?.min && parseFloat(value) < constraints.min) {
      return {
        status: 'INVALID',
        msg: $localize`:@@103:Value is less than ${constraints.min}`,
      };
    } else if (constraints?.max && parseFloat(value) > constraints.max) {
      return {
        status: 'INVALID',
        msg: $localize`:@@104:Value is greater than ${constraints.max}`,
      };
    }
    return null;
  }
  ...
  if (type.toLowerCase() == 'integer' || type.toLowerCase() == 'int') {
    if (!intRegex.test(value)) {
      ...
      return {
        status: 'INVALID',
        msg: $localize`:@@105:Value should not be negative`,
      };
  ...
  if (
    ...
    (type.toLowerCase() !== typeof value || !isNaN(parseInt(value)))
  ) {
    return {
      status: 'INVALID',
      msg: $localize`:@@102:Value is not a valid ${type}`,
