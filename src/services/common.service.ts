...
export class CommonService {
  private valueChangeSubject = new BehaviorSubject<ValueChangeInterface>({
    isValueChange: false,
    index: null,
    configData: undefined,
  });
  valueChangeMode$ = this.valueChangeSubject.asObservable();
  ...
  getModuleNamesForSelectedApp(list: SchemaList[], app: string): any[] {
    const data =
      list.filter(
        (item: SchemaList) => item.app.toUpperCase() == app.toUpperCase()
      ) ?? null;
    return [...data.map((item: SchemaList) => item.module)];
  }

  getVersionForSelectedSchemaData(
    list: SchemaList[],
    app: string,
    module: string
  ) {
    const data =
      list.filter(
        (item: SchemaList) =>
          item.app.toUpperCase() == app.toUpperCase() &&
  ...
  setEditMode(value: ValueChangeInterface) {
